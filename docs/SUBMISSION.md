...
```text
AI에게 맡긴 일: Flask·React의 Plan→Do→See 구현, 중복 완료 방지, JSON 내보내기, Docker·PostgreSQL 배포, 자동 검사와 공개 검증, 그리고 직접 만든 컨트롤의 구현과 대비 측정.
내가 판단한 일: 배포 조합(Render Free + Neon PostgreSQL)을 고르고, 화면은 «같은 종류가 둘로 갈리지 않게 한다»를 기준으로 정했다 — 목록 높이를 하나로 묶고, 브라우저가 그리던 달력과 드롭다운을 직접 그려 나머지 화면과 같은 토큰 위에 올렸다. 모바일에서 OS 시트를 잃는 건 값으로 치렀다.
AI 말을 안 들은 일: 할 일 목록의 높이 상한에서 AI는 «완료 그룹만 접으라»고 권했지만, 한 화면의 두 목록이 서로 다른 물건으로 읽히지 않도록 계획 목록과 같은 19rem으로 둘 다 묶었다. 좁은 화면에서 보이는 줄 수가 주는 건 알고 받아들였다.
```
...
User judgment: 배포 조합(Render Free + Neon PostgreSQL)을 고르고, 화면은 «같은 종류가 둘로 갈리지 않게 한다»를 기준으로 정했다 — 목록 높이를 하나로 묶고, 브라우저가 그리던 달력과 드롭다운을 직접 그려 나머지 화면과 같은 토큰 위에 올렸다. 모바일에서 OS 시트를 잃는 건 값으로 치렀다.

Rejected AI advice and reason: 할 일 목록의 높이 상한에서 AI는 완료 그룹만 접으라고 권했지만, 한 화면의 두 목록이 서로 다른 물건으로 읽히지 않도록 계획 목록과 같은 19rem으로 둘 다 묶었다. 좁은 화면에서 보이는 줄 수가 주는 건 알고 받아들였다.

집계 숫자를 자연어로 해석해 주는 기능을 넣지 않은 이전 판단도 유지한다 — 음수 차이를 무조건 시간 절약으로 오해할 수 있기 때문이다. 폼의 세 줄에는 더 최근이고 대가가 분명한 위 사례를 넣었다.
